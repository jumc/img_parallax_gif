...
# Horizontal scroll
	frames = []
	z_height, z_width = layers[0][0].shape[:2]
	cuts = []
	...
	for i in range(nframes):
		...
		for j in range(nlayers):
			...
			if((cuts[j][i] - cut_w) >= 0):
				...
				beta = cut_w
			layer_frame = layers[j][0][cut_h:(-cut_h), cut_w - beta:-(cut_w + beta)]
			# Merge acquired layer_frame and append to frames
			cv2.addWeighted(curr_frame, 0.5, layers[j][0], 0.5, 0, curr_frame) # Problem: counting alpha as black
...
#http://www.nooganeer.com/his/projects/image-processing/making-a-gif-with-opencv-and-scikit-image-in-python/
    ...
    plt.show()
    #im_ani.save('parallax.gif',writer="imagemagik")
